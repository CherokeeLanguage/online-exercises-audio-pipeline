...
from parsing.root_frequencies import count_roots_with_dict

from parsing.utils import SYLLABLE_VOWELS
from .prefixes import ParsedWord, Prefix, PronominalPrefix, all_parses, is_valid
...
def roots_for_row(word: VerbRow) -> List[ParsedWord]:
    ...
    parsed_forms: Dict[str, Tuple[str, List[ParsedWord]]] = {
        name: (
            ...
            [
                parse
                for parse in all_parses(strip_tone(form))
                if next(
                    (
                        True
        ...
        for name, form, expected_person, expected_set in [
            ("First present", word.first_present, "1", ""),
            ("Second command", word.second_command, "2", ""),
            ("Third habitual", word.third_incompletive_habitual, "3", ""),
            ("Third present", word.third_present, "3", ""),
            ("Third completive past", word.third_completive_past, "3", "B"),
            ("Third infinitive", word.third_infinitive, "3", "B"),
        ]
    }
...
def main():
    ...
    for parse in all_parses("udayilv"):
        print(is_valid(parse), parse)
    ...
    counts = count_roots_with_dict(
        ...
        sentences=(word.sentence.phonetics for _id, word in data.items()),
    )
    ...
    #         row = data[root_id]
    #         print(f"{start+i+1}: {row.third_present} has {count} occurances")
    #         print(f"\t{row.third_present} / {row.definition}")
    ...
    #     print()
    #     for word in dict_word.sentence.phonetics.split(" "):
    #         word = re.sub(r'[ \*\.,\?"]', "", word)
    #         found = []
    #         for parses in all_parses(word):
    #             if parses.root in root_based_dict:
    #                 # print(parses)
    #                 for found in root_based_dict[parses.root]:
    #                     print(parses, "\t", found.definition)
    ...
    for id, dict_word in data.items():
        if (
            ...
            continue
        example_forms = re.findall(r"\*([^\*]*)\*", dict_word.sentence.phonetics)
        print(dict_word.sentence.phonetics, example_forms)
